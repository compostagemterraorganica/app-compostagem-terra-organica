...
const { sendUserInvitation, sendAdminInvitation } = require('../email/email.service');
...
async function createUser(payload) {
  ...
  const { name, email, description, centralIds, isAdministrator: grantAdmin } = parsed.data;
  ...
  try {
    ...
    const rolesJson = grantAdmin ? JSON.stringify(['administrator']) : '[]';
    ...
    const userResult = await client.query(
      `INSERT INTO users (id, name, email, description, password, roles_json, registered_at, raw_json, created_at, updated_at)
       VALUES (
        ...
        $2, $3, $4, NULL, $5::jsonb, NOW(), '{}'::jsonb, NOW(), NOW()
       )
       RETURNING id, name, email, description, registered_at, roles_json`,
      [parsed.data.id || null, name, email, description || null, rolesJson]
    );
  ...
  try {
    if (grantAdmin) {
      await sendAdminInvitation({
        ...
        name: user.name,
        email: user.email,
        adminPanelUrl: env.adminPanelUrl
      });
    } else {
