...
// Rota para listar registros planos para exportação (data postagem, central, volume, link) — ordenado por data
router.get('/export-records', async (req, res) => {
  try {
    ...
    const centralById = {};
    centrals.forEach(c => {
      centralById[c.id] = c.title?.rendered || c.name || c.title || '';
    });

    const CENTRAL_4757_MIN = Date.UTC(2020, 0, 1);

    const records = [];
    for (const v of allVerifications) {
      const centralId = v.meta?.central;
      const centralName = centralId != null ? (centralById[centralId] || '') : '';
      const metaData = v.meta?.data;
      const t = parseMetaDateToTime(metaData);
      if (t === null) continue;

      if (String(centralId) === '4757' && t < CENTRAL_4757_MIN) continue;

      const volume = parseFloat(v.meta?.volume || 0);
      const linkVideo = v.meta?.['link-do-video'] || v.link || '';

      const d = new Date(t);
      const dd = String(d.getDate()).padStart(2, '0');
      const mm = String(d.getMonth() + 1).padStart(2, '0');
      const yyyy = d.getFullYear();
      const dataPostagem = dd + '/' + mm + '/' + yyyy;
    ...
    records.sort((a, b) => a.dataPostagemSort - b.dataPostagemSort);
    ...
    logger.error('Erro ao gerar export-records:', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
...
<body>
    ...
    <div class="action-buttons">
        ...
        <button class="export-btn" id="exportBtn" onclick="exportData()">⬇️ Exportar Dados</button>
    </div>
    ...
    <script>
        ...
        async function exportData() {
            ...
            if (!btn) btn = document.querySelector('.export-btn');
            ...
            try {
                ...
                const resp = await fetch('/analytics/export-volume-report?format=json&t=' + Date.now());
                const json = await resp.json();
                if (!json.success) throw new Error(json.error || 'Erro ao carregar dados para exportação');

                let aoa;
                if (json.report && Array.isArray(json.report.sheet) && json.report.sheet.length > 0) {
                    aoa = json.report.sheet;
                } else {
                    const records = Array.isArray(json.data) ? json.data : [];
                    aoa = [['Data da postagem', 'Central', 'Volume', 'Link do vídeo']];
                    records.forEach(r => {
                        aoa.push([
                            r.dataPostagem || '',
                            r.central || '',
