...
function mapPublicCentral(row, { verifications = [], listingImageUrl = null } = {}) {
  ...
  return {
    ...
    excerpt: pickExcerpt(raw),
    content_html: pickContentHtml(raw),
    image_url: imageUrl,
    address: meta.address ? decodeHtmlEntities(String(meta.address)) : null,
    city: meta.city ? decodeHtmlEntities(String(meta.city)) : null,
    state: meta.state ? decodeHtmlEntities(String(meta.state)) : null,
    email: meta.email ? decodeHtmlEntities(String(meta.email)) : null,
