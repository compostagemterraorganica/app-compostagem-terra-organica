...
const { uploadBuffer, deleteObject, isGcsConfigured } = require('../../config/gcs');
...
async function createMedia(file, userId) {
  ...
  if (!isGcsConfigured()) {
    throw new HttpError(500, 'GCS nao configurado para upload de midia');
  }
...
async function deleteMedia(id) {
  ...
  if (row.storage_key && isGcsConfigured()) {
